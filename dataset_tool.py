...
def create_from_images(tfrecord_dir, image_dir, shuffle, add_condition):
    ...
    img_path = os.path.join(image_dir, df['Filenames'][0])
    ...
    for i in range(len(df["Filenames"])):
        img_path = os.path.join(image_dir, df["Filenames"].iloc[i])
        img = np.asarray(PIL.Image.open(img_path))
        if channels == 1:
    ...
    with TFRecordExporter(tfrecord_dir, len(df)) as tfr:
        ...
        for idx in range(order.size):
            ...
            img_path = os.path.join(image_dir, df["Filenames"].iloc[order[idx]])
            img = np.asarray(PIL.Image.open(img_path))
            if channels == 1:
                img = img[np.newaxis, :, :]  # HW => CHW
            else:
